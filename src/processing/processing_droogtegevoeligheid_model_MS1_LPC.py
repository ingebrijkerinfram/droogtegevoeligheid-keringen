...
def verwelkingspunt(df, TAW, D100=45, B=2):
    ...
    # Cover op 1 april en 1 oktober (None als die datum niet voorkomt)
    cover_1_april = None
    cover_1_oktober = None
    if not df[df['Date'].dt.month.eq(4) & df['Date'].dt.day.eq(1)].empty:
        cover_1_april = df.loc[(df['Date'].dt.month == 4) & (df['Date'].dt.day == 1), 'living_plant_cover'].iloc[0]
